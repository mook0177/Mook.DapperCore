...
namespace Mook.DapperCore
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        /// <param name="transaction">The transaction to run under, null (the default) if none</param>
        /// <returns>Entity of T</returns>
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object filterCondition, bool orCondition = false, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            return connection.Query<T>(builderSql.ToString(), filterCondition, transaction, commandTimeout: commandTimeout);
        }
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if updated, false if not found or not modified (tracked entities)</returns>
        public static bool Update<T>(this IDbConnection connection, object data, object filterCondition = null, bool updateSelf = false, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
        ...
        internal static List<string> GetParamNames(object o)
        {
            ...
            if (!paramNameCache.TryGetValue(o.GetType(), out List<string> paramNames))
            {
                ...
                foreach (var prop in o.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(p => p.GetGetMethod(false) != null))
                {
                    ...
                    if (attribs.FirstOrDefault() is not IgnorePropertyAttribute attr || (!attr.Value))
                    {
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if deleted, false if not found</returns>
        public static bool Delete<T>(this IDbConnection connection, object filterCondition, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            ...
            var deleted = connection.Execute(sb.ToString(), filterCondition, transaction, commandTimeout);
            return deleted > 0;
        }
